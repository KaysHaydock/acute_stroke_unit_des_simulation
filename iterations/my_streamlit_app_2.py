...
with st.sidebar:    
    ...
    RUN_LENGTH = 365 * st.sidebar.slider("Simulation Run Length (years)", min_value=1, max_value=10, value=5)
...
if st.button("Run Simulation"):
    with st.spinner("Simulation running..."):  
        ...
        # st.write("Stroke Transfers to Rehab: ", results["stroke_transfer"]["rehab"])
        # st.write("Stroke Transfers to ESD: ", results["stroke_transfer"]["esd"])
        # st.write("Stroke Transfers to Other: ", results["stroke_transfer"]["other"])
        
        # st.write("TIA Transfers to Rehab: ", results["tia_transfer"]["rehab"])
        # st.write("TIA Transfers to ESD: ", results["tia_transfer"]["esd"])
        # st.write("TIA Transfers to Other: ", results["tia_transfer"]["other"])
        
        # st.write("Complex Neuro Transfers to Rehab: ", results["complex_neuro_transfer"]["rehab"])
        # st.write("Complex Neuro Transfers to ESD: ", results["complex_neuro_transfer"]["esd"])
        # st.write("Complex Neuro Transfers to Other: ", results["complex_neuro_transfer"]["other"])
        
        # st.write("Other Transfers to Rehab: ", results["other_transfer"]["rehab"])
        # st.write("Other Transfers to ESD: ", results["other_transfer"]["esd"])
        # st.write("Other Transfers to Other: ", results["other_transfer"]["other"])

        # Patient counts table
        patient_data = {
            "Category": ["Total Patients", "Stroke Patients", "TIA Patients", 
                         "Complex Neuro Patients", "Other Patients"],
            "Count": [results["n_patients"], results["n_stroke"], results["n_tia"], 
                      results["n_complex_neuro"], results["n_other"]]
        }
        patient_df = pd.DataFrame(patient_data)
        st.table(patient_df)  # Display as table

        # Transfers table
        transfers_data = {
            "Transfer Type": ["Rehab", "ESD", "Other"],
            "Total Transfers": [results["total_transfers"]["rehab"], 
                                results["total_transfers"]["esd"], 
                                results["total_transfers"]["other"]],
            "Stroke Transfers": [results["stroke_transfer"]["rehab"], 
                                 results["stroke_transfer"]["esd"], 
                                 results["stroke_transfer"]["other"]],
            "TIA Transfers": [results["tia_transfer"]["rehab"], 
                              results["tia_transfer"]["esd"], 
                              results["tia_transfer"]["other"]],
            "Complex Neuro Transfers": [results["complex_neuro_transfer"]["rehab"], 
                                        results["complex_neuro_transfer"]["esd"], 
                                        results["complex_neuro_transfer"]["other"]],
            "Other Transfers": [results["other_transfer"]["rehab"], 
                                results["other_transfer"]["esd"], 
                                results["other_transfer"]["other"]]
        }
